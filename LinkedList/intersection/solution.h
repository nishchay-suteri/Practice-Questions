...
class Solution{
    protected:
        virtual SinglyListNode<int>* intersection() = 0;
        SinglyListNode<int>* head1;
        SinglyListNode<int>* head2;
        void take_input();
    public:
...
class Result{
    public:
        SinglyListNode<int>* tail;
        int size;
        Result(SinglyListNode<int>* n,int s){
            this->tail = n;
            this->size = s;
        }
};
...
class OptimalSolution: public Solution{
    private:
        Result* get_size_and_tail(SinglyListNode<int>* node,int size);
        SinglyListNode<int>* intersection();
    public:
        OptimalSolution();
        void solve();
};
